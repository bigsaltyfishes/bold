...
pub const Bind = struct {
    ...
    pub fn updateSize(self: *Self, macho_file: *MachO) !void {
        ...
        for (objects.items) |index| {
            ...
            for (file.getAtoms()) |atom_index| {
                ...
                for (relocs) |rel| {
                    ...
                    const entry = Entry{
                        .target = rel.getTargetSymbolRef(atom.*, macho_file).unwrap().?,
        ...
        macho_file.dyld_info_cmd.bind_size = mem.alignForward(u32, @intCast(self.size()), @alignOf(u64));
    }
    ...
    fn finalizeSegment(entries: []const Entry, ctx: *MachO, writer: anytype) !void {
        ...
        var count: usize = 0;
        var skip: u64 = 0;
        var target: ?Symbol.UnwrappedRef = null;
        ...
        while (i < entries.len) : (i += 1) {
            ...
            log.debug("{x}, {d}, {x}, {x}, {t}", .{ offset, count, skip, addend, state });
...
pub const WeakBind = struct {
    ...
    pub fn updateSize(self: *Self, macho_file: *MachO) !void {
        ...
        try self.finalize(gpa, macho_file);
        macho_file.dyld_info_cmd.weak_bind_size = mem.alignForward(u32, @intCast(self.size()), @alignOf(u64));
    }
