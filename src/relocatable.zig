pub fn flush(macho_file: *MachO) !void {
...

    {
        ...
        try macho_file.segments.append(macho_file.allocator, .{
...
fn calcSectionSizes(macho_file: *MachO) !void {
...

    {
        ...
        for (slice.items(.atoms), 0..) |atoms, i| {
            ...
            macho_file.thread_pool.spawnWg(&wg, calcSectionSizeWorker, .{ macho_file, @as(u8, @intCast(i)) });
        }

        if (macho_file.eh_frame_sect_index) |_| {
            macho_file.thread_pool.spawnWg(&wg, calcEhFrameSizeWorker, .{macho_file});
        }
        ...
        macho_file.thread_pool.spawnWg(&wg, MachO.updateLinkeditSizeWorker, .{ macho_file, .data_in_code });
    }
...
fn calcSectionSizeWorker(macho_file: *MachO, sect_id: u8) void {
    ...
    doWork(macho_file, header, atoms) catch |err| {
        macho_file.fatal("failed to calculate size of section '{s},{s}': {s}", .{
            header.segName(),
            header.sectName(),
            @errorName(err),
        });
...
fn calcEhFrameSizeWorker(macho_file: *MachO) void {
    ...
    doWork(macho_file, header) catch |err| {
        macho_file.fatal("failed to calculate size of section '__TEXT,__eh_frame': {s}", .{
            @errorName(err),
        });
        _ = macho_file.has_errors.swap(true, .seq_cst);
    };
...
fn writeSections(macho_file: *MachO) !void {
    ...
    for (slice.items(.header), slice.items(.out), slice.items(.relocs)) |header, *out, *relocs| {
        ...
        try out.resize(macho_file.allocator, header.size);
        const padding_byte: u8 = if (header.isCode() and cpu_arch == .x86_64) 0xcc else 0;
        @memset(out.items, padding_byte);
        try relocs.resize(macho_file.allocator, header.nreloc);
    }
    ...
    var wg: WaitGroup = .{};
    {
        ...
        if (macho_file.eh_frame_sect_index) |_| {
            macho_file.thread_pool.spawnWg(&wg, writeEhFrameWorker, .{macho_file});
        }
...
fn sortRelocs(macho_file: *MachO) void {
    ...
    var wg: WaitGroup = .{};
    {
        ...
        for (macho_file.sections.items(.relocs)) |*relocs| {
            macho_file.thread_pool.spawnWg(&wg, worker, .{relocs.items});
        }
    }
}
...
fn writeSectionsToFile(macho_file: *MachO) !void {
    ...
    const slice = macho_file.sections.slice();
    for (slice.items(.header), slice.items(.out), slice.items(.relocs)) |header, out, relocs| {
        try macho_file.file.pwriteAll(out.items, header.offset);
        try macho_file.file.pwriteAll(mem.sliceAsBytes(relocs.items), header.reloff);
    }

    try macho_file.writeDataInCode();
    try macho_file.file.pwriteAll(mem.sliceAsBytes(macho_file.symtab.items), macho_file.symtab_cmd.symoff);
    try macho_file.file.pwriteAll(macho_file.strtab.items, macho_file.symtab_cmd.stroff);
}
...
fn writeCompactUnwindWorker(macho_file: *MachO, object: *Object) void {
    ...
    object.writeCompactUnwindRelocatable(macho_file) catch |err| {
        macho_file.fatal("failed to write '__LD,__eh_frame' section: {s}", .{@errorName(err)});
        _ = macho_file.has_errors.swap(true, .seq_cst);
    };
}
...
fn writeEhFrameWorker(macho_file: *MachO) void {
    ...
    eh_frame.writeRelocs(macho_file, buffer.items, relocs.items) catch |err| {
        macho_file.fatal("failed to write '__TEXT,__eh_frame' section: {s}", .{@errorName(err)});
        _ = macho_file.has_errors.swap(true, .seq_cst);
    };
}
...
fn writeLoadCommands(macho_file: *MachO) !struct { usize, usize } {
    ...
    try macho_file.file.pwriteAll(buffer, @sizeOf(macho.mach_header_64));
...
fn writeHeader(macho_file: *MachO, ncmds: usize, sizeofcmds: usize) !void {
    ...
    try macho_file.file.pwriteAll(mem.asBytes(&header), 0);
}
